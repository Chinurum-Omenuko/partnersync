<script lang="ts">
	...
	const { user } = $props();
	let isExternal = $state(user?.claims?.role === 'org:partner' || false);
	let position = $state('Staff');

	let isAdmin = $state(user?.claims?.role === 'org:admin' || false);
...
<div class="nav min-w-100 hidden h-full flex-[0.5] bg-white md:block">
